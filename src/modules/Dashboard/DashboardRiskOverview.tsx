...
const DashboardRiskOverview: React.FC<DashboardRiskOverviewProps> = React.memo(({ t, stats, colors }) => {
  ...
  return (
    <div className="glass-card p-8 flex flex-col min-w-0">
      ...
      <div className="flex-1 min-h-[250px] w-full min-w-0 relative">
        ...
        <div className="absolute inset-0 flex items-center justify-center flex-col pointer-events-none">
          <span className="text-3xl font-black text-[var(--color-text-main)]">{formatNumber(stats.risks.summary.total)}</span>
          <span className="text-xs font-bold text-[var(--color-text-muted)] uppercase tracking-widest">{t('dashboard.totalRisks')}</span>
        </div>
      </div>
