      ...
      >
        <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div className="space-y-1">
            <h3 className="text-2xl font-bold text-slate-800">{program.program_title}</h3>
        ...
        <div className="flex-1 overflow-y-auto p-8 custom-scrollbar space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="p-6 bg-slate-50 rounded-3xl space-y-2">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{t('program.auditObjective')}</span>
              <p className="text-sm font-bold text-slate-700 leading-relaxed">{program.audit_objective}</p>
            </div>
            <div className="p-6 bg-slate-50 rounded-3xl space-y-2">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{t('program.auditScope')}</span>
              <p className="text-sm font-bold text-slate-700 leading-relaxed">{program.audit_scope}</p>
            </div>
            <div className="p-6 bg-slate-50 rounded-3xl space-y-2">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{t('program.keyRisks')}</span>
              <p className="text-sm font-bold text-slate-700 leading-relaxed">{program.key_risks}</p>
            </div>
          </div>

          <div className="space-y-4">
            <h4 className="text-lg font-bold text-slate-800 px-2">{t('program.proceduresList')}</h4>
            <div className="overflow-hidden border border-slate-100 rounded-3xl">
              <table className="w-full text-start border-collapse">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-100">
                    <th className="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest text-center w-16">{t('program.procedureNumber')}</th>
                    <th className="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest text-start">{t('program.auditStep')}</th>
                    <th className="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest text-start">{t('program.testType')}</th>
                    <th className="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest text-start">{t('program.expectedEvidence')}</th>
                  </tr>
                </thead>
