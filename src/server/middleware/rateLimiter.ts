...
import type { RateLimiterOptions } from '../types/middleware';
...
export function createRateLimiter(options: RateLimiterOptions = {}) {
  ...
  return (req: Request, res: Response, next: NextFunction): void => {
    ...
    // If there are timestamps, the window resets when the oldest timestamp expires
...
export function getRateLimitCount(key: string): number {
  ...
  if (!entry) return 0;
  const now = Date.now();
  // We don't know the window here, so just return raw count
