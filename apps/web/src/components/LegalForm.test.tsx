...
/**
 * Component Tests - LegalForm submission failure handling (Req 4.1, 4.3, 4.4, 4.5)
 *
 * Verifies the AuditTaskForm-style submission pattern:
 *   - a failed save surfaces Submit_Error_Feedback (toast.error),
...
function renderForm() {
  ...
  const onClose = vi.fn();
  const result = render(<LegalForm onSuccess={onSuccess} onClose={onClose} />);
  const form = result.container.querySelector('form') as HTMLFormElement;
  return { onSuccess, onClose, form, ...result };
}

describe('LegalForm submission failure handling', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockApi.get.mockResolvedValue({ data: [] });
  });

  it('shows Submit_Error_Feedback (toast.error) when the save fails', async () => {
    ...
    await waitFor(() => {
      expect(mockToast.error).toHaveBeenCalledWith('errorOccurred');
    });
  ...
  it('prevents a second submission while the first is in progress', async () => {
    ...
    // Wait until isSubmitting is reflected in the disabled control.
    await waitFor(() => {
      expect(screen.getByRole('button', { name: 'common.loading' })).toBeDisabled();
    });

    // A second submit while submitting must be ignored by the isSubmitting guard.
    fireEvent.submit(form);
