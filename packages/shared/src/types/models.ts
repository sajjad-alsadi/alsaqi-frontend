 ...
 * Mirrors the grouped backend response consumed by the Dashboard module.
 */
export interface DashboardStats {
  audits: {
    total: number;
    completed: number;
    progress_by_type: AuditProgressByType[];
  };
  findings: {
    summary: {
      open: number;
...
/**
 * User-management policy settings returned by `GET /v1/user-management-settings`.
